...
- 2026-09-04: migration 118 公司假日納入計算：`holidays` 表改為 `company_id/holiday_date/holiday_name/type`（原空表欄位 date/name，114 與 attendance_public 一直查錯欄位→缺時稽核從未成功）；`is_company_holiday()` 只在「無排班日」排除假日（排班優先）；`count_employee_workdays`／`get_company_monthly_attendance`／payroll.js `computeEmployeeExpectedDays(…, holidaySet)` 三處同規則。缺時通知門檻 `missing_work_hours_min_minutes`（預設 60）進 scan 與 preview 兩處。月統計缺工分鐘走 `get_company_monthly_missing_minutes`（逐日呼叫 `calculate_missing_work_hours`，單一來源）。前端讀假日一律走 `get_company_holidays` RPC，不開 anon 政策。2026 假日只匯入大正（本米假日營業）。
- 2026-09-04: migration 119 假日維護走 RPC（upsert/delete_company_holiday，has_company_access manager）＋ admin.html 地點管理頁假日卡片；2027 假日已種子。approve_leave_request 改新簽章含 p_company_id/p_line_user_id，approver_id 由 RPC 解析，舊簽章只回 deprecated_signature（沿用 113 對舊簽章的做法）。缺時 LINE 提醒 9/4 起開啟（大正，門檻 60 分）。
- 2026-09-09: migration 120 月統計 leave_days／absent_days 改 double precision（PostgREST 回 JSON 數字，前端加總不必改），半天 0.5、時數假 hours/8、同日多張上限 1；新增喪假 bereavement（薪資只對 personal 扣款，喪假不扣）。後台補登假單的正確路徑＝以員工 line_user_id 呼叫 submit_leave_request 再以管理員 approve（classifier 會擋多列直接 INSERT leave_requests，且 RPC 路徑有重疊防呆與 days 計算）。加班若主管未按確認就是 0，補救走 confirm_daily_overtime。
- 2026-09-09: migration 121 補登不算遲到／早退——以 check_in_location／check_out_location 是否為補登標記（makeup punch／admin makeup／補打卡）做**側別**判斷，不用 is_manual（整列旗標分不出哪一側）。四處同步：calculate_missing_work_hours 歸 0、月統計次數排除、兩支補登 RPC 覆寫時清旗標、既有旗標資料修正。大正 late_threshold_minutes 改 0。部署大檔用 `supabase db query --linked -f`。
- 2026-09-09: migration 122 早退＝下班卡早於班表結束（扣容忍），不再限 2 小時窗口（035 的窗口是為凌晨誤判，跨日班另有分支保留）。月統計遲到次／早退次前端改讀 get_company_monthly_missing_minutes 的 late_count／early_count（請假、補登、容忍一致處理），打卡旗標只當 RPC 失敗的退路。業主規則：中午走人有請假＝半天假、沒請假＝早退。
- 2026-09-12: migration 123 quick_check_in 遲到／早退比較前先 date_trunc('minute')（容忍 0 分時 08:00:47 不得算遲到）。規則：08:01:00 起遲到、16:59:xx 早退。calculate_missing_work_hours 原本就 FLOOR 分鐘，兩邊現在一致。
- 2026-09-12: RLS 階段 1 employees（migration 124）：寫入一律走 admin_create/update/delete_pending_employee（has_company_access manager＋欄位白名單＋role 需 is_company_admin_caller）、register_employee（公開但伺服器強制 pending/user）、set_my_preferred_language（本人）。common.js 提供 rpcUpdateEmployee/rpcCreateEmployee/rpcDeletePendingEmployee 回 {data,error} 讓既有 throw 寫法沿用。anon 的 employees 寫入 grant 與全開政策撤掉，讀取不動。之後新增 employees 寫入功能一律用這組 RPC；rls-locked-tables.test.js 的 WRITE_LOCKED 會抓多行鏈式直接寫入。
