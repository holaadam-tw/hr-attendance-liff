...
function helpers(userAgent) {
  return new Function('navigator', `
    const CHECKIN_HEALTH_VERSION = '20260810.1';
    ${grab('makeHealthResult')}
    ${grab('makeHealthFailure')}
    ${grab('getHealthPlatform')}
    ${grab('buildHealthSafeSummary')}
    return { makeHealthResult, makeHealthFailure, getHealthPlatform, buildHealthSafeSummary };
  `)({ userAgent });
}
...
check('定位逾時仍歸類定位問題', ios.makeHealthFailure('定位', { code: 3 }, startedAt).name === '定位');
...
check('安全摘要包含版本與錯誤碼', summary.includes('20260810.1') && summary.includes('photo_encode_failed'));
...
check('定位檢查重用正式定位 helper', healthCode.includes('resolveGpsLocation()'));
