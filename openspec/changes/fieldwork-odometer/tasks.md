...
## Task 1：migration 093 建表與加欄位 ✅（2026-07-15 已套正式庫並驗證）
...
**驗收條件**：
- [x] SQL 可重複執行不報錯（IF NOT EXISTS 齊全）
- [x] anon REST 驗證：`field_work_trips` 可 select（回 `[]` 或資料，不是 42P01）；`field_work_logs?select=trip_id,odometer_reading,segment_km` 不回 42703
- [x] 實測 partial unique index：同員工同日 insert 兩筆 status='open' → 第二筆報 23505
...
**驗收條件**：
- [x] logic walkthrough：未出發／進行中／已收工 三狀態渲染正確，重新整理後狀態不變
- [x] 結束讀數 < 最後讀數點 → 擋下並提示
- [x] GPS 失敗（模擬 getGPS reject）→ 出發登錄仍成功、lat/lng 為 null
- [x] 照片上傳失敗 → 提示但讀數仍送出成功
- [x] insert 帶 company_id（grep 確認）；查詢帶 employee_id filter（多租戶）
- [x] `node --check`（抽出 inline script）語法通過
...
**驗收條件**：
- [x] logic walkthrough ≥3 情境：①第一站（前一點=trip start）②第二站（前一點=前站讀數/GPS）③無行程（舊流程不變、mileage 照舊）
- [x] 讀數 < 前一讀數點 → 擋；segment>200 → confirm
- [x] haversineKm 單元驗證：台北101(25.0340,121.5645)→台北車站(25.0478,121.5170) ≈ 5.0±0.3 km（bash node 實跑給輸出）
- [x] 無行程模式完全不受影響（既有欄位 id 全部保留，回歸：離開打卡自填 mileage 仍可送出）
- [x] `node --check` 通過；qa_check 0 FAIL；npm test 52 通過
...
**驗收條件**：
- [x] trips 查詢帶 company_id filter；logs 查詢維持 employees!inner 模式（Hook 無新警告）
- [x] 舊資料（無 trip_id）明細不顯示里程表區塊、不誤報警示（null 安全）
- [x] 警示函數 bash node 實跑 4 案例：低於直線×0.8 ⚠️／正常區間無警示／高於直線×3+5 ⚠️／gps null 無警示
- [x] CSV 欄位順序與表頭正確（node 實跑產出範例列）
- [x] qa_check 0 FAIL；npm test 52 通過

## Task 5：整合驗證與收尾 ✅（REST 實測完成、測試資料已清除）
...
**驗收條件**：
- [x] REST 實測輸出貼在驗收證據（trip insert → log insert 含 segment_km → 管理端查詢帶出 → 測試資料已刪）
- [x] rls-checker 報告 PASS
- [x] qa_check 0 FAIL；npm test 52 通過；Hook 無新警告
- [x] git commit 訊息含：改了什麼／為什麼／測試了什麼；push origin dev（不動 main）
