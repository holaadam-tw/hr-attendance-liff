...
function taipeiMinutes(iso) {
    const hm = new Date(iso).toLocaleTimeString('en-GB', { timeZone: 'Asia/Taipei', hour: '2-digit', minute: '2-digit' });
...
export async function loadAuditData() {
    ...
    try {
        const [empRes, salaryRes, attRes, leaveRes, otRes, schedRes] = await Promise.all([
            ...
            sb.from('leave_requests').select('employee_id, days, leave_type, leave_period, leave_hours, start_date, end_date, employees!inner(company_id)').eq('employees.company_id', window.currentCompanyId).eq('status', 'approved').lte('start_date', endDate).gte('end_date', startDate),
        ...
        const rows = (empRes.data || []).map(emp => {
            ...
            atts.filter(a => a.check_in_time).forEach(a => {
                ...
                if (late > 0) {
                    ...
                    if (afternoon) afternoonCnt++;
                    const hm = new Date(a.check_in_time).toLocaleTimeString('en-GB', { timeZone: 'Asia/Taipei', hour: '2-digit', minute: '2-digit' });
                    lateList.push({ date: a.date.slice(5), hm, min: late, afternoon, manual: !!a.is_manual });
                }
            });
            lateList.sort((x, y) => x.date.localeCompare(y.date));

            const distinctDates = new Set(atts.filter(a => a.check_in_time).map(a => a.date));
            const actualDays = distinctDates.size;
            const lv = leaveMap[emp.id] || { annualH: 0, otherH: 0 };
            const annualDays = Math.round(lv.annualH / 8 * 10) / 10;
            const leaveHours = lv.otherH;
            const otHours = Math.round((otMap[emp.id] || 0) * 10) / 10;
            const leaveDaysTotal = (lv.annualH + lv.otherH) / 8;
            ...
            return {
                ...
                lateMin, afternoonCnt, annualDays, leaveHours, otHours,
                actualDays, expected, absent, lateList
            };
        }).filter(Boolean).sort((a, b) => a.no.localeCompare(b.no));

        renderAuditView(rows, { year, month });
...
function renderAuditView(rows, meta) {
    ...
    const tot = {
        people: rows.length,
        lateMin: rows.reduce((s, r) => s + r.lateMin, 0),
        afternoon: rows.reduce((s, r) => s + r.afternoonCnt, 0),
        otHours: Math.round(rows.reduce((s, r) => s + r.otHours, 0) * 10) / 10
    };
    ...
    if (sumEl) {
        ...
        sumEl.innerHTML = `
            ...
            <div style="text-align:center;"><div style="font-size:11px;opacity:.85;">遲到總分鐘</div><div style="font-size:22px;font-weight:900;">${tot.lateMin}</div></div>
            <div style="text-align:center;"><div style="font-size:11px;opacity:.85;">下午打卡(需剔除)</div><div style="font-size:22px;font-weight:900;">${tot.afternoon} 筆</div></div>
            <div style="text-align:center;"><div style="font-size:11px;opacity:.85;">加班總時數</div><div style="font-size:22px;font-weight:900;">${tot.otHours}</div></div>`;
    }
    ...
    let html = `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table style="border-collapse:collapse;width:100%;min-width:640px;font-size:13px;">
        <thead><tr style="background:#F1F5F9;color:#64748B;font-size:11px;">
            <th style="${cellL}text-align:left;">員工</th>
            <th style="${cell}">遲到(分)</th><th style="${cell}">特休</th>
            <th style="${cell}">請假</th><th style="${cell}">加班</th>
            <th style="${cell}">出勤</th><th style="${cell}">缺勤</th>
        </tr></thead><tbody>`;
    rows.forEach(r => {
        const lateColor = r.lateMin >= 120 ? '#DC2626' : r.lateMin >= 30 ? '#B45309' : '#0F172A';
        const lateTxt = r.lateMin > 0
            ? `<span style="font-weight:800;color:${lateColor};">${r.lateMin}</span>${r.afternoonCnt > 0 ? `<span style="color:#B45309;font-weight:800;" title="含 ${r.afternoonCnt} 筆下午打卡，分鐘灌大">＊</span>` : ''}`
            : `<span style="${zero}">0</span>`;
        const annualTxt = r.annualDays > 0 ? `${r.annualDays}天` : `<span style="${zero}">-</span>`;
        const leaveTxt = r.leaveHours > 0 ? fmtHours(r.leaveHours) : `<span style="${zero}">-</span>`;
        const otTxt = r.otHours > 0 ? `${r.otHours}hr` : `<span style="${zero}">-</span>`;
        const absTxt = r.monthly ? (r.absent > 0 ? `<span style="color:#DC2626;">${r.absent}天</span>` : `<span style="${zero}">0</span>`) : `<span style="${zero}">—</span>`;
        html += `<tr>
            <td style="${cellL}"><div style="font-weight:700;color:#0F172A;">${esc(r.name)}</div><div style="font-size:10px;color:#94A3B8;">${esc(r.no)}${r.dept ? ' · ' + esc(r.dept) : ''}${r.monthly ? ' · 月薪' : ''}</div></td>
            <td style="${cell}">${lateTxt}</td><td style="${cell}">${annualTxt}</td>
            <td style="${cell}">${leaveTxt}</td><td style="${cell}">${otTxt}</td>
            <td style="${cell}">${r.actualDays}</td><td style="${cell}">${absTxt}</td>
        </tr>`;
    });
    html += `<tr style="background:#F8FAFC;font-weight:800;">
        <td style="${cellL}">合計（${tot.people} 人）</td>
        <td style="${cell}">${tot.lateMin}</td>
        <td style="${cell}" colspan="5"></td></tr>`;
    html += `</tbody></table></div>`;

    // 遲到明細（每筆打卡時刻 + 分鐘）
    const lateRows = rows.filter(r => r.lateMin > 0);
    if (lateRows.length) {
        html += `<div style="margin-top:18px;font-weight:800;font-size:14px;color:#0F172A;">遲到打卡明細</div>
            <div style="font-size:11px;color:#94A3B8;margin-bottom:8px;">每筆＝當日打卡時刻與距 08:00 的分鐘；<span style="color:#B45309;">橘色</span>＝中午後才打卡（多為漏打上班卡，分鐘會灌很大，請人工剔除）；補＝補打卡</div>`;
        lateRows.forEach(r => {
            const chips = r.lateList.map(p => {
                const susp = p.afternoon ? 'background:#FEF3C7;color:#B45309;' : 'background:#F1F5F9;color:#475569;';
                const dash = p.manual ? 'border:1px dashed #CBD5E1;' : 'border:1px solid transparent;';
                return `<span style="display:inline-block;font-size:11.5px;padding:3px 7px;border-radius:6px;${susp}${dash}font-variant-numeric:tabular-nums;margin:2px;"><span style="opacity:.6;">${p.date}</span> ${p.hm} <b>+${p.min}分</b>${p.manual ? '<sup style="font-size:8px;">補</sup>' : ''}</span>`;
            }).join('');
            html += `<div style="padding:8px 0;border-bottom:1px solid #F1F5F9;">
                <div style="font-size:12px;font-weight:700;color:#0F172A;">${esc(r.name)} <span style="color:#94A3B8;font-weight:400;">${esc(r.no)}</span> <span style="color:#DC2626;">· 合計 ${r.lateMin} 分</span></div>
    ...
    html += `<div style="margin-top:18px;background:#FFFBEB;border:1px solid #FDE68A;border-radius:12px;padding:14px;font-size:12px;color:#78350F;line-height:1.7;">
        ...
        1. <b>遲到＝每筆打卡時間 − 08:00 的分鐘總和</b>，一律從 08:00 起算、不扣寬限（全員上班基準 08:00）。<br>
        2. 標「＊/橘色」＝中午後才打卡（多為漏打上班卡），分鐘會灌成幾百分，<b>請人工剔除</b>再核對。<br>
        3. <b>特休以天、請假以時數</b>（整天＝8hr、半天＝4hr、時假用實際時數）。App 請假多為整天登記，沒有分鐘級細節。<br>
        4. <b>加班需員工在系統線上送單</b>核准後才有；目前查無核准加班時多顯示「-」，實際加班請對照現場紀錄。<br>
        5. 缺勤僅<b>月薪制</b>自動計（應出勤−出勤−請假）；時薪制顯示「—」，薪水以工時計。</div>`;
...
function isBenmiPayrollCompany() {
    return window.currentCompanyId === BENMI_COMPANY_ID;
}
...
function calcEmployeePayroll(emp, ss, atts, leaves, otHours, year, month, payrollSettings, expectedDaysOverride, options = {}) {
    const _ps = payrollSettings || {};
    const skipAutoAbsence = options.skipAutoAbsence === true;
    ...
    const expectedDays = (!skipAutoAbsence && expectedDaysOverride !== undefined && expectedDaysOverride !== null) ? expectedDaysOverride : (_ps.work_days_per_month || 22);
    const displayExpectedDays = skipAutoAbsence ? null : expectedDays;
    ...
    // 缺勤扣款：本米排班制不自動扣；一般公司月薪制依應出勤自動計算。
    const autoAbsenceEnabled = !skipAutoAbsence && salaryType === 'monthly';
    const absentDays = autoAbsenceEnabled ? Math.max(0, expectedDays - actualDays - leaveDays) : null;
    const absenceDed = autoAbsenceEnabled ? Math.round(dailyRate * absentDays) : 0;
    ...
    return {
        ...
        calculation_details: {
            ...
            expected_days: displayExpectedDays, actual_days: actualDays, absent_days: absentDays,
            ...
            auto_absence_enabled: autoAbsenceEnabled
        }
    };
}
...
function renderAllPayrollTable() {
    ...
    payrollEmployees.forEach(emp => {
        ...
        else baseDesc = cd.expected_days === null || cd.expected_days === undefined
            ? `固定 ${formatNT(emp.base_salary)}`
            : `固定 ${formatNT(emp.base_salary)} / 應出勤${cd.expected_days}天`;
...
function renderPayrollCard(empId) {
    ...
    const incomeItems = [
        { label: '基本薪資', value: emp.base_salary, desc: cd.salary_type === 'monthly' ? (cd.expected_days === null || cd.expected_days === undefined ? '固定月薪（排班制）' : `固定月薪（應出勤 ${cd.expected_days}天）`) : cd.salary_type === 'daily' ? `日薪 ${cd.original_base?.toLocaleString()} × ${cd.actual_days}天` : `時薪 ${cd.original_base?.toLocaleString()} × ${cd.total_work_hours}h` },
    ...
    const deductItems = [
        { label: '缺勤扣款', value: emp.absence_deduction, desc: cd.absent_days > 0 ? `${cd.absent_days}天 × 日薪 ${cd.daily_rate?.toLocaleString()}` : (cd.auto_absence_enabled ? '無缺勤' : '排班制暫不自動計算') },
    ...
    const expectedDaysText = cd.expected_days === null || cd.expected_days === undefined ? '--' : `${cd.expected_days}天`;
    const absentDaysText = cd.absent_days === null || cd.absent_days === undefined ? '--' : `${cd.absent_days}天`;
    const absentColor = (cd.absent_days || 0) > 0 ? '#DC2626' : '#059669';

    html += `<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin-bottom:16px;">`;
    html += `<div style="text-align:center;background:#F8FAFC;border-radius:10px;padding:8px 4px;"><div style="font-size:10px;color:#94A3B8;">應出勤</div><div style="font-size:15px;font-weight:800;color:#64748B;">${expectedDaysText}</div></div>`;
    ...
    html += `<div style="text-align:center;background:#F8FAFC;border-radius:10px;padding:8px 4px;"><div style="font-size:10px;color:#94A3B8;">缺勤</div><div style="font-size:15px;font-weight:800;color:${absentColor};">${absentDaysText}</div></div>`;
