...
- **瀏覽器回歸（2026-09-15，Puppeteer＋LIFF mock 走正式站，以 Adam 身分）**：新增員工、編輯電話、寬鬆定位、設管理員／取消、綁 LINE（未綁定分頁）、離職／恢復、人力管理頁工時模式儲存、QR 自助登記×2（以不存在的 LINE 身分）、審核通過（改派工號 E826）、拒絕登記（刪除）、員工切換語言 vi-VN→zh-TW，**全部 DB 驗證通過**（腳本 scratchpad e2e_124*.js；toast 攔截未成功但以 DB 結果為準）。測試員工 TEST-124／E826 與 makeup 軌跡已清除，在職 23 人不變，Adam 語言還原 zh-TW。
- 下一張：schedules（排班）。
...
- migration 123（以正式庫 122 版為基底只加一行）：`v_tw_time := date_trunc('minute', now() AT TIME ZONE 'Asia/Taipei')::time`，遲到／早退比較都吃分鐘。08:00:xx 不遲到、08:01:00 起遲到；16:59:xx 早退、17:00:xx 不早退。
- 回填：大正 9/9 起 is_late=true 且缺時計算 0 分、非補登上班卡 → false（4 筆：上述 3 筆＋林駿璋 9/9 12:56 有上午假）。
- 驗證：`tests/late-minute-granularity.test.js` 15 項（反向 3 失敗）；npm test 20 套件；部署後函式含 date_trunc、9/9 起仍為遲到的只剩真的 08:10 以後的 4 筆。
- 純資料庫，不需合併 main。
