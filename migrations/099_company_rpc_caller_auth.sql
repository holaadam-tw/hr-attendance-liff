-- ============================================================
-- 099: 三支公司層級 RPC 補上呼叫者身分驗證（阻斷跨公司資料外洩）
--
-- 問題（2026-08-03 實測確認）：
--   get_company_daily_attendance / get_company_monthly_attendance /
--   get_weekly_schedules 都是 SECURITY DEFINER 且 GRANT 給 anon，但參數只有
--   p_company_id，完全不驗呼叫者是誰。任何人拿公開的 anon key（就寫在
--   common.js 裡）加上任一 company_id，即可用 owner 權限 dump 別家公司的
--   整月出勤明細與班表——這比表層 RLS 缺口更嚴重，因為 SECURITY DEFINER
--   本來就繞過所有表層保護。
--   實測證據：以 anon key 帶本米 company_id 呼叫，成功取得本米員工姓名、
--   部門與逐日打卡時間。
--
-- 本次修正：
--   1. 新增 has_company_access(p_line_user_id, p_company_id, p_require_manager)
--      helper：檢查該 LINE 使用者是否為該公司在職員工（require_manager 時
--      限 admin/manager/公務機），或為該公司的平台管理員。已 REVOKE anon，
--      避免被當成「某人是否屬於某公司」的查詢管道。
--   2. 三支 RPC 各加 p_line_user_id TEXT DEFAULT NULL 參數與前置驗證。
--      出勤兩支要求管理身分（對齊前端 canReadAttendanceOverview 的判斷：
--      admin/manager 或公務機帳號）；週班表只要求是該公司在職員工
--      （沿用「週班表公司內公開查看」的既有設計）。
--
-- ⚠️ 過渡設計：p_line_user_id 為 NULL 時維持舊行為，不驗證。
--   原因：前端傳新參數前，函式必須先具備該參數，否則 PostgREST 會找不到
--   對應函式而讓打卡總覽整頁壞掉。部署順序必須是
--   「099（可選）→ 前端改傳 → 100（改必填）」，本檔是第一步，
--   **這一步還沒有把洞補起來**，要等 100 才算真正關閉。
--
-- 注意：參數數量改變必須 DROP 舊簽章再建，否則新舊 overload 並存會讓
--   PostgREST 報 Could not choose the best candidate function（098 的教訓）。
--   函式本體除了插入驗證區塊之外，完全沿用現行正式庫版本，未做任何更動。
-- ============================================================
...
-- 只給 SECURITY DEFINER RPC 內部使用；直接開放會變成身分探測管道
REVOKE ALL ON FUNCTION has_company_access(TEXT, UUID, BOOLEAN) FROM PUBLIC, anon, authenticated;

-- ============================================================
-- get_company_daily_attendance
-- ============================================================
DROP FUNCTION IF EXISTS get_company_daily_attendance(uuid, date);
...
BEGIN
    ...
    -- 099: 呼叫者身分驗證。p_line_user_id 為 NULL 時維持舊行為（過渡期，
    -- 讓尚未更新的前端不中斷）；migration 100 會改為必填。
...
GRANT EXECUTE ON FUNCTION get_company_daily_attendance(uuid, date, text) TO anon, authenticated;

-- ============================================================
-- get_company_monthly_attendance
-- ============================================================
DROP FUNCTION IF EXISTS get_company_monthly_attendance(uuid, integer, integer);
...
GRANT EXECUTE ON FUNCTION get_company_monthly_attendance(uuid, integer, integer, text) TO anon, authenticated;
...
BEGIN
    ...
    -- 讓尚未更新的前端不中斷）；migration 100 會改為必填。
    IF p_line_user_id IS NOT NULL
       AND NOT has_company_access(p_line_user_id, p_company_id, false) THEN
        RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
    END IF;
...
GRANT EXECUTE ON FUNCTION get_weekly_schedules(uuid, date, text) TO anon, authenticated;
