...
--
-- 變更：
-- 1. 一般日班：下班打卡限 shift_end ~ shift_end + N 小時（預設 3）
--    超過 → 拒絕，回傳「已超過下班打卡時間，請申請補卡」
-- 2. 跨日班（is_overnight=true）：保留查昨天記錄的邏輯
-- 3. 一般日班：不再查昨天的未下班記錄
--    隔天打卡 = 新的上班，昨天記錄保持 check_out_time=NULL
-- 4. 新增 system_settings key: checkout_time_limit_hours（預設 3）
-- ============================================================
...
BEGIN
    ...
    IF v_existing.id IS NULL THEN
        DECLARE
            ...
            v_yesterday_schedule RECORD;
            v_yesterday_overnight BOOLEAN := false;
        BEGIN
            -- 查昨天未完成的記錄
            ...
            IF v_yesterday_rec.id IS NOT NULL THEN
                -- 查昨天的排班是否為跨日班
                SELECT COALESCE(st.is_overnight, false) INTO v_yesterday_overnight
                FROM schedules s
