-- 039: 下班打卡記錄 GPS 座標和地點名稱
--
-- 問題：下班打卡只記 check_out_time，不記錄地點和座標
-- 修正：
--   1. attendance 表加 checkout_latitude / checkout_longitude
--   2. quick_check_in RPC 下班流程記錄 GPS + 地點名稱（不驗證範圍）
...
-- Step 2: 更新 RPC（在 037 基礎上加入下班地點記錄）
DROP FUNCTION IF EXISTS quick_check_in(TEXT, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, TEXT);
...
BEGIN
    ...
    IF p_action = 'check_in' THEN
        IF v_existing.id IS NOT NULL AND v_existing.check_out_time IS NULL THEN
            IF v_existing.date = v_today THEN
                RETURN jsonb_build_object('success', false, 'error', '今日已打上班卡');
            ELSE
                RETURN jsonb_build_object('success', false, 'error', '昨日尚未下班打卡，請先完成下班打卡');
            END IF;
        END IF;
        v_do_check_in := true;
    ELSIF p_action = 'check_out' THEN
    ...
    IF v_do_check_out THEN
        ...
        IF v_locations IS NOT NULL AND jsonb_typeof(v_locations) = 'array' THEN
            ...
            LOOP
                ...
                IF v_dist <= COALESCE((v_loc->>'radius')::double precision, 100) AND v_dist < v_min_dist THEN
                    v_min_dist := v_dist;
                    v_checkout_location := v_loc->>'name';
                END IF;
            END LOOP;
        END IF;
        ...
        UPDATE attendance SET
            ...
            check_out_location = v_checkout_location,
        ...
        RETURN jsonb_build_object(
            'success', true,
            'type', 'check_out',
            'location_name', v_checkout_location,
            'check_in_location', v_existing.check_in_location,
            'is_early_leave', v_is_early_leave,
