-- 031_fix_quick_check_in_exception.sql
-- 修正下班打卡顯示「已打過上班卡」的問題
-- 根本原因：EXCEPTION WHEN unique_violation 在整個函數層級
-- 若下班打卡因任何原因進入 INSERT 分支，會觸發 unique_violation
-- 修正：將 INSERT 放在 BEGIN...EXCEPTION 子區塊，不影響 UPDATE 流程
-- 額外修正：加入 p_check_type 參數，前端可明確指定上班/下班
...
BEGIN
    ...
    IF v_existing IS NOT NULL THEN
        ...
        -- 即使 check_in_time 為 null（異常情況），只要記錄存在就執行下班
        -- 查詢排班下班時間
    ...
    EXCEPTION WHEN unique_violation THEN
        ...
        IF v_existing IS NOT NULL AND v_existing.check_out_time IS NULL THEN
            UPDATE attendance SET
                check_out_time = v_now,
                total_work_hours = CASE
                    WHEN v_existing.check_in_time IS NOT NULL
