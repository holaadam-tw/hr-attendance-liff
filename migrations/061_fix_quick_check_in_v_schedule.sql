-- ============================================================
-- 061: 修復 quick_check_in v_schedule "record not assigned yet"
--
-- Bug: shift_mode='fixed' 且 fixed_shift_start/end 為 NULL 時，
--      v_schedule 從未被 SELECT INTO 賦值，
--      存取 v_schedule.id → 報錯 "record is not assigned yet"
--
-- 修正 1：DB 預設值 + 補既有資料
--   - fixed_shift_start DEFAULT '08:00'
--   - fixed_shift_end DEFAULT '17:00'
--   - 補上既有 NULL 的固定班員工
--
-- 修正 2：RPC 防呆
--   - v_schedule_found BOOLEAN 旗標取代直接存取未賦值 RECORD
--   - 下班：schedules → fixed_shift_end → system_settings → '17:00'
--   - 上班：schedules → fixed_shift_start → system_settings → '08:00'
--   - INSERT 用 CASE WHEN v_schedule_found 保護 v_schedule 存取
-- ============================================================

-- ===== 修正 1：DB 預設值 + 補既有資料 =====

ALTER TABLE employees ALTER COLUMN fixed_shift_start SET DEFAULT '08:00';
ALTER TABLE employees ALTER COLUMN fixed_shift_end SET DEFAULT '17:00';

UPDATE employees
SET fixed_shift_start = '08:00',
    fixed_shift_end = '17:00'
WHERE shift_mode = 'fixed'
  AND (fixed_shift_start IS NULL OR fixed_shift_end IS NULL);

-- ===== 修正 2：quick_check_in RPC 防呆 =====
...
BEGIN
    ...
    IF v_do_check_out THEN
        -- 取得班別資訊
        -- 優先順序：查 schedules → 員工 fixed_shift_end → 系統預設
        v_schedule_found := false;

        -- 1. 嘗試查排班表（排班制員工，或固定班但想 fallback）
    ...
    -- 班別判定：查 schedules → 員工 fixed_shift_start → 系統預設
    v_schedule_found := false;
