-- ============================================================
-- 123: 遲到／早退判定以「分」為單位（秒數捨去）
--
-- 背景（2026-09-11 業主）：「08:00 打卡出現遲到？8:01 才算遲到。」
-- 大正 9/9 起 late_threshold_minutes=0，quick_check_in 的判定 `v_tw_time > v_shift_start` 帶秒數比較，
-- 08:00:47 > 08:00:00 → 被記遲到。9/10 豐豪 08:00:47、簡杏如 08:00:49、9/11 黃律瑋 08:00:04 皆誤判。
-- 缺時計算（calculate_missing_work_hours）本來就用 FLOOR(分鐘)，這三筆算出來是 0 分，只有打卡當下的旗標錯。
--
-- 本檔：
-- A. quick_check_in（以正式庫 122 版為基底、只加一行）：v_tw_time 先 date_trunc('minute') 再做遲到／早退比較。
--    效果：08:00:xx 不遲到、08:01:00 起遲到；16:59:xx 早退、17:00:xx 不早退。
-- B. 回填：大正 2026-09-09 起（容忍改 0 之後）is_late=true 但缺時計算遲到 0 分、且上班卡非補登 → is_late=false。
--
-- 部署方式：僅 migration 檔；正式庫套用依業主 2026-09-11「幫忙處理一下」指示。
-- ============================================================
...
WHERE e.id = a.employee_id
  ...
  AND a.date >= DATE '2026-09-09'
  AND a.is_late = true
  AND a.check_in_time IS NOT NULL
  AND NOT public.is_makeup_location(a.check_in_location)
  AND COALESCE((public.calculate_missing_work_hours(a.employee_id, a.date) ->> 'late_minutes')::INTEGER, -1) = 0;

-- ===== 部署後驗證（唯讀） =====
-- 1. pg_get_functiondef(quick_check_in) 含 date_trunc('minute'
-- 2. SELECT is_late FROM attendance WHERE date='2026-09-11' AND employee_id=黃律瑋;  → false
