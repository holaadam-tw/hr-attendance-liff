-- 029_loyalty_consolidated.sql
-- 整合：loyalty_members phone/expiry + loyalty_redemptions + expiry trigger

-- loyalty_members 加欄位（IF NOT EXISTS 安全重複執行）
ALTER TABLE loyalty_members ADD COLUMN IF NOT EXISTS phone TEXT;
ALTER TABLE loyalty_members ADD COLUMN IF NOT EXISTS expiry_date DATE;

-- phone partial unique（同公司不重複，但允許 null）
DROP INDEX IF EXISTS idx_loyalty_members_company_phone;
