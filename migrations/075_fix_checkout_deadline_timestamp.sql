-- ============================================================
-- 075: fix checkout deadline timestamp comparison
--
-- Problem:
-- quick_check_in compared current TIME with shift_end + hours.
-- For 21:30 + 4h, PostgreSQL TIME wraps to 01:30, so same-day
-- checkout around 22:00 was incorrectly treated as expired.
--
-- Fix:
-- Build a Taiwan local TIMESTAMP deadline from attendance.date
-- + shift_end + checkout_time_limit_hours, then compare with the
-- current Taiwan local TIMESTAMP.
-- ============================================================
...
BEGIN
    ...
    v_checkout_limit := COALESCE(v_setting_val, '4')::integer;
