-- 037: quick_check_in 加入 p_action 參數，區分上班/下班動作
--
-- 問題：上班/下班共用同一個 RPC，自動判斷導致：
--   - 連點上班兩次 → 第二次變下班（錯誤）
--   - 沒上班就點下班 → 變上班（錯誤）
--
-- 修正：
--   p_action = 'check_in'  → 只允許上班打卡
--   p_action = 'check_out' → 只允許下班打卡
--   p_action = NULL         → 向後兼容，保留自動判斷

-- 先刪除舊的 5 參數版本（避免 overload ambiguity）
