-- 022_add_early_leave.sql
-- 1. attendance 加 is_early_leave 欄位
-- 2. quick_check_in RPC 下班打卡加入早退判定

ALTER TABLE attendance
ADD COLUMN IF NOT EXISTS is_early_leave BOOLEAN DEFAULT FALSE;

-- 重建 quick_check_in：加入早退判定
...
BEGIN
    ...
    IF v_existing IS NOT NULL AND v_existing.check_in_time IS NOT NULL THEN
        ...
        ELSE
            -- 無排班 → system_settings default_work_end（預設 17:00）
        ...
        -- 判定早退：打卡時間 < 下班時間 - 容忍分鐘
        IF v_now_time < (v_shift_end - (v_early_threshold || ' minutes')::interval) THEN
    ...
    IF v_now_time > (v_shift_start + (v_late_threshold || ' minutes')::interval) THEN
        v_is_late := true;
    END IF;
